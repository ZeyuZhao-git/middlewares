package cn.zeke.middleware.test.interfaces;
...
public class UserController {
     ...
     */
    @DoRateLimiter(permitsPerSecond = 0.5, returnJson = "{\"code\":\"1111\",\"info\":\"令牌不足\"}")
    @WhitelistFilter(key = "userId", returnJson = "{\"code\":\"1111\",\"info\":\"非白名单可访问用户拦截！\"}")
    @DoHystrix(timeout = 350, returnJson = "{\"code\":\"1111\",\"info\":\"调用方法超过350毫秒，熔断返回！\"}")
    @DoMethodExt(methodName = "blacklist", returnJson = "{\"code\":\"1111\",\"info\":\"自定义校验方法拦截，不允许访问！\"}")
    ...
    public boolean blacklist(@RequestParam String userId) {
        if ("aaa".equals(userId) || "222".equals(userId)) {
